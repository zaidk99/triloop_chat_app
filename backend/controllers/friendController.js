...
//   const senderuser = await User.findById(senderId).select("fullName username _id");
...
//   const acceptedFriend = await User.findById(request.from).select("_id fullName username");
...
// 🔴 SEND FRIEND REQUEST - ADD io PARAMETER & EMIT
export const sendFriendRequest = async (req, res, io) => {
  const senderId = req.userId;
  const receiverId = req.params.receiverId;
...
// 🔴 RESPOND TO FRIEND REQUEST - ADD io PARAMETER & EMIT
...
// 🔴 CANCEL FRIEND REQUEST - ADD io PARAMETER & EMIT
...
// GET FRIEND REQUESTS (no changes needed)
export const getFriendRequests = async (req, res) => {
  const userId = req.userId;
...
// GET FRIENDS LIST (no changes needed)
export const getFriends = async (req, res) => {
  const userId = req.userId;
...
// SEARCH USERS (no changes needed)
export const searchUsers = async (req, res) => {
  const userId = req.userId;
  const q = (req.query.q || "").trim();
  ...
  try {
    ...
    const results = users.map((u) => {
      let relation = "none";
      if (friendSet.has(u._id.toString())) relation = "friend";
      else if (outgoingSet.has(u._id.toString()))
        relation = "pending_outgoing";
      else if (incomingSet.has(u._id.toString()))
