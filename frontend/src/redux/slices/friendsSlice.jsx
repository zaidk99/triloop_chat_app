...
//     friendRequestRecieved: (state , action) =>{
...
//       if(friend && !state.friends.some((f)=>f.id === friend._id)){
...
// ... your existing thunks stay exactly the same ...
export const fetchRequests = createAsyncThunk(
  "friends/fetchRequests",
  async () => {
    const { data } = await axios.get("/friends/requests");
...
const friendSlice = createSlice({
  ...
  reducers: {
    ...
    friendRequestReceived: (state, action) => {
      ...
      // Add to incoming if not already exists
      if (!state.incomingRequests.some((r) => r.id === newRequest.id)) {
        state.incomingRequests.unshift(newRequest);
      }
    },
    ...
    friendRequestAccepted: (state, action) => {
      ...
      // Add to friends list (use friend object if provided, else just ID)
...
// 🔴 EXPORT ALL REDUCERS - THIS WAS MISSING:
export const {
  resetFriendState,
